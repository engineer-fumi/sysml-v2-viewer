...
- **ライン編集**: 線を選択すると経由点ハンドルが表示され、
  ◌(セグメント中点)をドラッグで経由点追加、●をドラッグで移動、
  ダブルクリックで削除。スタイルは線ごとに **直線 / 折れ線(直角) / 曲線** を
  切替可能。経由点は端点ボックスの中点を基準とした相対座標で保存されるため、
  ボックスを動かしても線形が追従
- **描画ファイル**: 配置(ボックスのオフセット)と線形(スタイル・経由点)は
  ワークスペースの `.sysml-layout.json`(サイドカー)に図ルート単位で自動保存。
...
code --install-extension sysml-v2-viewer-0.5.0.vsix
```
