...
- **図の種類ごとの表示**: コマンド「SysML: 図の種類を選んで開く」または
  パネル内のセレクタで切り替え。種類ごとに別パネルで並べて開けます
  - **全体図** — モデル全体(構造・振る舞いのすべて)
  - **ブロック定義図 (BDD)** — 構造定義 (part def 等) と特化(白抜き三角)・
    コンポジション(黒菱形)関係
  - **内部ブロック図 (IBD)** — 内部構造を持つブロックの part 構成と
    connect / flow / port
  - **要求図** — 要求(doc 本文付き)と satisfy / verify 関係
  - **ユースケース図** — ユースケース(楕円)と actor(関連線で接続)、
    perform / include 関係
  - **状態遷移図** — 状態機械(状態と transition)のみ
  - **アクティビティ図** — アクションと succession / flow
  - **シーケンス図** — ライフラインとメッセージ(flow / succession を時系列に表示)
...
- **サイズ変更**: 右下・右上ハンドルのドラッグでボックスを拡大
  (右上は上方向に広がる)
- **port の配置**: port をドラッグでボックス四辺の任意位置へ移動
- **線のルーティング**: 線をドラッグで中継点を追加して折れ線化。
  中継点はドラッグ移動・ダブルクリック削除。線を選択するとツールバーで
  **線種 (直線 / 折れ線(直角) / 曲線)** を切替・「経由点クリア」も可能
- **描画ファイル**: 配置・サイズ・port 位置・線の経路/線種はワークスペースの
  `.sysml-layout.json`(サイドカー)に
